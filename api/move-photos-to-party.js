// Vercel Serverless Function — moves verified invitation photos from the
// memories collection to the separate live-party collection without deleting
// or changing the original uploaded image bytes.
...
export default async function handler(req, res) {
  ...
  try {
    const memoriesResponse = await fetch(
      `https://api.cloudinary.com/v1_1/${cloudName}/resources/image/tags/${encodeURIComponent(MEMORIES_TAG)}?max_results=500`,
      { headers },
    )
    if (!memoriesResponse.ok) return res.status(memoriesResponse.status).json({ error: 'Unable to verify memories photos', details: await memoriesResponse.text() })
    const memories = await memoriesResponse.json()
    const knownIds = new Set((memories.resources || []).map(resource => resource.public_id))
    if (!publicIds.every(id => knownIds.has(id))) return res.status(404).json({ error: 'A selected photo was not found in the memories collection' })
    ...
    // Add first, then remove. The original asset stays intact throughout.
    await tagRequest(LIVE_PARTY_TAG, 'add')
    await tagRequest(MEMORIES_TAG, 'remove')
    return res.status(200).json({ success: true, moved: publicIds.length, public_ids: publicIds })
  } catch (error) {
    return res.status(500).json({ error: error.message || 'Unable to move selected photos' })
  }
}
